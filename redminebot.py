...
print("Hello User")
run = input("What do you want to run today : <issue or time>")
...
if run == 'issue':
    ...
    cursor.execute("""INSERT INTO ISSUES (ISSUE_ID, SUBJECT, HOURS ,ACTIVITY ,TEAM) VALUES (?,?,?,?,?)""", (issue.id, subject, round(time), Activity, Team))
    conn.commit()
    print("Issue saved")
    conn.close()
elif run == 'time' :
    ...
    for wordlist in cursor.execute("SELECT * FROM ISSUES"):
        ...
        issue = red.time_entry.create(issue_id=issue_id, spent_on=today, hours=time_spent, activity_id=activity_id[activity], custom_fields=[{'id': 64, 'value': team}])
